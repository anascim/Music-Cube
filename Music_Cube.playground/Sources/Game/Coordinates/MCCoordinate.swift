...
public struct MCCoordinate {
    ...
    public func getDominant() -> (Int, MCOrientation) {
        for i in 0...2 {
            if self[i] == 0 {
                switch i {
    ...
    public func nextCoord(on direction: MCDirection) -> MCCoordinate {
        var nextCoord = self
    ...
    public func checkOverstep(on direction: MCDirection) -> (Bool) {
        let (_, face) = getDominant()
    ...
    public func retreatDominant(_ coord: MCCoordinate) -> MCCoordinate {
        var newCoord = self
        for i in 0...2 {
            if newCoord[i] == 0 {
                newCoord[i] = 1
